...
import { IRuneUtxo, IUtxo } from "../type";
...
// Calc Tx Fee
export const calculateTxFee_v2 = (psbt: Bitcoin.Psbt, feeRate: number) => {
  const tx = new Bitcoin.Transaction();
...
// Calc Tx Fee
export const transferAllAssetsFeeCalc = (psbt: Bitcoin.Psbt, feeRate: number, thresHoldValue: number) => {
  const tx = new Bitcoin.Transaction();
  ...
  tx.addOutput(psbt.txOutputs[0].script, psbt.txOutputs[0].value);
  console.log("tx virtial size ==> ", tx.virtualSize() + COSIGNATURE_SIZE * thresHoldValue)
  

  return Math.floor((tx.virtualSize() + COSIGNATURE_SIZE * thresHoldValue) * feeRate);
};
...
export const getRuneIdByName = async (runeName: string) => {
  const url = `${OPENAPI_UNISAT_URL}/v1/indexer/runes/${runeName}/info`;
  const config = {
    headers: {
      Authorization: `Bearer ${OPENAPI_UNISAT_TOKEN}`,
    },
